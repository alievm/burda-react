...
import {
    ...
    Space,
    Table,
    notification, Spin, Flex, DatePicker, Segmented, Divider, Form, Radio, Collapse, Pagination
} from 'antd';
...
import {
    createRecieverCategory,
    fetchRecieverCategories,
    updateRecieverCategory,
    deleteRecieverCategory
} from "../../service/directories/receiverCategoryService.js";
import {createSubEntity, deleteSubEntity, updateSubEntity} from "../../service/finance/Sources/subConsumption.js";
...
import {createWarehouse, deleteWarehouse, fetchWarehouse, updateWarehouse} from "../../service/warehousesService.jsx";
...
const WareHouse = () => {
    ...
    const fetchMainCategoriesData = async () => {
        ...
        try {
            const response = await fetchWarehouse(); // Assuming fetchSeasons fetches the data from the API
    ...
    const handleEdit = (record) => {
        ...
        setEditCategoryTitle(record.name);
        setEditCategoryDesc(record.description);
    ...
    const handleUpdateCategory = async () => {
        ...
        try {
            const updatedCategory = await updateWarehouse(editCategoryId, editCategoryTitle, editCategoryDesc);
    ...
    const handleCreateNewCategory = async () => {
        ...
        try {
            const newCategory = await createWarehouse(newCategoryTitle, newCategoryDesc);
    ...
    const handleDeleteCategory = async (categoryId) => {
        try {
            const success = await deleteWarehouse(categoryId);
            ...
            <div className="bg-white rounded-xl border border-gray-200 border-solid shadow-sm h-full min-h-[85vh]">
                <div className="flex flex-col bg-white">
                    <div className="flex gap-4 px-6 py-5 max-md:flex-wrap max-md:px-5">
                        ...
                        <Modal
                            centered
                            title={editCategoryId ? 'Изменить склад' : ' Добавить новый склад'}
                    ...
                    <Layout>
                        <Content style={contentStyle}>
                            <DndContext modifiers={[restrictToVerticalAxis]} onDragEnd={onDragEnd}>
                                <SortableContext items={filteredDataSource.map((i) => i.key)}
