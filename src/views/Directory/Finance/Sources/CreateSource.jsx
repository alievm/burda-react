...
import {
    ...
    Space,
    Table,
    notification, Spin, Collapse, Flex, Segmented, Form, DatePicker, Divider, Radio, Pagination
} from 'antd';
...
import {createWidth, fetchWidth, updateWidth, deleteWidth} from "../../../../service/directories/widthService.js";
import {createSource, fetchSource, updateSource, deleteSource} from "../../../../service/finance/Sources/sourceService.js";
...
const CreateSources = () => {
    ...
    const fetchMainCategoriesData = async () => {
        ...
        try {
            const response = await fetchSource(); // Assuming fetchSeasons fetches the data from the API
            ...
            if (Array.isArray(response)) {
                ...
                const formattedCategories = response.map((category) => ({
                    ...
                    desc: category.desc,
                    created_at: moment(category.created_at).utcOffset('+0500').format('YYYY-MM-DD HH:mm:ss'),
    ...
    const handleUpdateCategory = async () => {
        ...
        try {
            const updatedCategory = await updateSource(editCategoryId, editCategoryTitle, editCategoryDesc);
    ...
    const handleCreateNewCategory = async () => {
        ...
        try {
            const newCategory = await createSource(newCategoryTitle, newCategoryDesc);
    ...
    const handleDeleteCategory = async (categoryId) => {
        try {
            const success = await deleteSource(categoryId);
            ...
            <div className="bg-white rounded-xl border border-gray-200 border-solid shadow-sm h-full min-h-[85vh]">
                <div className="flex flex-col bg-white">
                    <div className="flex gap-4 px-6 py-5 max-md:flex-wrap max-md:px-5">
                        ...
                        <Modal
                            centered
                            title={editCategoryId ? 'Изменить Источник' : 'Добавить новый Источник'}
