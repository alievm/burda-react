...
import {fetchDensities, updateDensity} from "../../../service/directories/densityService.js";
...
const Density = () => {
    ...
    const [checkedList, setCheckedList] = useState(['operation', 'title']);
    ...
    const columns = [
      ...
      <img loading="lazy"
           src="https://cdn.builder.io/api/v1/image/assets/TEMP/82ca4f2625b4345dff2b3b5c99dc8a526ded8cd54dc4ce408ec4b6c7e754f7f2?apiKey=0e60d26ffe404316aa35b6241738714a&"
    ...
    const save = async (key) => {
        try {
            ...
            if (index > -1) {
                ...
                setData(newData);
                setEditingKey('');
                await updateDensity(item.id, row);
                messageApi.success('Плотность обновлена!');
            } else {
                newData.push({...row});
                setData(newData);
                setEditingKey('');
                const createdDensity = await createDensity(row.title);
                messageApi.success(`Добавлена новая плотность: ${createdDensity.title}`);
            }
        } catch (errInfo) {
    ...
    const fetchData = () => {
        ...
        fetchDensities()
            .then((response) => {
                setData(response.data); // Assuming response.data contains array of densities
                setLoading(false);
                setTableParams((prev) => ({
                    ...prev,
                    pagination: {
                        ...prev.pagination,
                        total: response.data.length, // Set total count of data
                    },
                }));
            })
            .catch((error) => {
