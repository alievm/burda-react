...
export const createSeason= async (title, desc) => {
    try {
        const response = await axios.post('https://devapi.istamgroup.com/api/production_seasons', {
            title: title,
            desc: desc
        });
...
export const fetchSeasons = async () => {
    try {
        const response = await axios.get('https://devapi.istamgroup.com/api/production_seasons');
        if (response.status === 200) {
...
export const deleteSeason = async (id) => {
    try {
        const response = await axios.delete(`https://devapi.istamgroup.com/api/production_seasons/${id}`);
...
export const updateSeason = async (id, newTitle, desc) => {
    try {
        const response = await axios.put(`https://devapi.istamgroup.com/api/production_seasons/${id}`, {
            title: newTitle,
            desc: desc
        });
