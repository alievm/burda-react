...
export const createWidth= async (title, desc) => {
    try {
        const response = await axios.post('https://devapi.istamgroup.com/api/widths', {
            title: title,
            desc: desc
        });
...
export const fetchWidth = async () => {
    try {
        const response = await axios.get('https://devapi.istamgroup.com/api/widths');
        if (response.status === 200) {
...
export const deleteWidth = async (id) => {
    try {
        const response = await axios.delete(`https://devapi.istamgroup.com/api/widths/${id}`);
...
export const updateWidth = async (id, newTitle, desc) => {
    try {
        const response = await axios.put(`https://devapi.istamgroup.com/api/widths/${id}`, {
            title: newTitle,
            desc: desc
        });
