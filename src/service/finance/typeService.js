...
export const createType = async (title) => {
    try {
        // Fetch the existing currencies to determine the highest current code
        const existingCurrencies = await fetchTypes();
        ...
        const response = await axios.post('https://devapi.istamgroup.com/api/types', {
            title: title,
            code: newCode
        });
...
export const fetchTypes = async () => {
    try {
        const response = await axios.get('https://devapi.istamgroup.com/api/types');
        if (response.status === 200) {
...
export const deleteType = async (id) => {
    try {
        const response = await axios.delete(`https://devapi.istamgroup.com/api/types/${id}`);
...
export const updateType = async (id, newTitle, code) => {
    try {
        const response = await axios.put(`https://devapi.istamgroup.com/api/types/${id}`, {
            title: newTitle,
            code: code
        });
